...
def deploy_base_snapshot(): # REVIEW removed "{SUDO}" from all lines below
    ...
    os.system(f"{btrfs} sub snap / /.snapshots/rootfs/snapshot-chr0")
...
def deploy_to_common():
    ...
    os.system("umount /var")
    os.system(f"mount {os_root} -o subvol=@var{distro_suffix},compress=zstd,noatime /.snapshots/var/var-deploy")
    ...
    os.system("cp -r --reflink=auto /.snapshots/var/var-0/. /.snapshots/rootfs/snapshot-deploy/var/")
...
def post_bootstrap(super_group): # REVIEW removed "{SUDO}" from all lines below
  ...
  # TODO may write these in python
    ...
    os.system(f"sed -i '0,/@var{distro_suffix}/ s|@var{distro_suffix}|@.snapshots{distro_suffix}/var/var-deploy|' /etc/fstab")
